...
Action向量（**单次 pick_and_place 原语**，一步 env.step 完成抓取+放置）：
  ...
  执行入口唯一：`_execute_pick_and_place`（方块→抓取→抬起→**同一函数内**桶上方→放置；
  非两次独立 `_execute_pick`/`_execute_place`）。向桶过渡前会 `set_start_state_to_current_state()`。
...
class SagittariusPickPlaceEnv(gym.Env):
    ...
    def _execute_pick_and_place(
        ...
        **唯一**底层原语 `pick_and_place`：一条连续执行链，**不**拆成「先 pick 再 place」两次调用。
        ...
        流程：移动到目标方块 → 抓取 → 抬起验证 → 附着 → 直接过渡到桶上方 → 下放 → 开爪 → 抬起。

        抓取段结束后在同一函数内继续放置段；在首次向桶运动时调用
        `set_start_state_to_current_state()`，使规划从**当前持块姿态**延续，
        而不是第二次独立「place 原语」从零开始碰撞判定的语义。
        ...
        self._attach_held_block_to_scene(pick_color)

        # ── 持块连续运动：桶上方 → 放置（同一原语，非第二次「place」调用）──
        try:
        ...
        if not self._move_to_xy(
            place_x, place_y, TABLE_Z + PLACE_H, yaw_bin,
            orientation_mode="horizontal", ignore_block_color=ign,
        ):
            return {"pick_ok": True, "place_ok": False, "done": False}
        self._pause_robot_stage()
        ...
        if pick_color:
            self._detach_held_block_from_scene(pick_color)
        self._holding_color = None

        place_ok = self._move_to_xy(
            place_x, place_y, TABLE_Z + APPROACH_H, yaw_bin,
            orientation_mode="horizontal",
        )
        self._pause_robot_stage()
        if not place_ok:
            rospy.logwarn("[Env] 放置后抬升路径曾返回 False，忽略并继续判定 done")
