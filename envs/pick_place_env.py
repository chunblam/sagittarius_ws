...
# 抓取末端姿态：topdown = 优先「从上往下」夹（少占 xy 平面、类似俯视）；yaw = 仅绕世界 Z 转（侧向夹、占空间大）
GRASP_ORIENTATION_MODE = "topdown"  # "topdown" | "yaw"
# 俯视姿态四元数 (x,y,z,w)，与 URDF 中 ee_link 定义有关；规划失败会自动回退到 yaw/neutral
# 默认：绕世界 X 轴 180°，使末端 Z 大致朝下（若仿真中姿态不对，可改为 (0,0.707,0,0.707) 等）
TCP_TOPDOWN_QUAT_XYZW = (1.0, 0.0, 0.0, 0.0)
TCP_TOPDOWN_QUAT_ALT_XYZW = (0.0, 0.7071067811865476, 0.0, 0.7071067811865476)  # 绕 Y +90° 备选
...
class SagittariusPickPlaceEnv(gym.Env):
    ...
    def _move_to_xy(self, x: float, y: float, z: float, yaw: float = 0.0,
                    ...
                    orientation_mode: str = "yaw") -> bool:
        """
        规划并执行到目标位姿（末端笛卡尔）。

        orientation_mode:
          - "yaw"：仅绕世界 Z 的 yaw（侧向夹取，xy 占用大）
          - "topdown"：先尝试俯视四元数（TCP_TOPDOWN_QUAT_*），再回退 yaw/neutral
        ...
        if orientation_mode == "topdown":
            qtd = TCP_TOPDOWN_QUAT_XYZW
            _add_attempt("topdown_x180", qtd[0], qtd[1], qtd[2], qtd[3], z)
            _add_attempt("topdown_x180_z+up", qtd[0], qtd[1], qtd[2], qtd[3], z_up)
            qalt = TCP_TOPDOWN_QUAT_ALT_XYZW
            _add_attempt("topdown_y90", qalt[0], qalt[1], qalt[2], qalt[3], z)
            _add_attempt("topdown_y90_z+up", qalt[0], qalt[1], qalt[2], qalt[3], z_up)
